  ...
  private def getIsCompressedOops: Boolean = {
    ...
    try {
      ...
      case e: Exception => {
        ...
        Console.println("Failed to check whether UseCompressedOops is set; assuming " + guessInWords)
        return guess
      }
    }
  }
  ...
  private def visitArray(array: AnyRef, arrayClass: Class[_], state: SearchState) {
    ...
    if (elementClass.isPrimitive) {
      ...
      if (length <= ARRAY_SIZE_FOR_SAMPLING) {
        ...
        val drawn = new util.HashSet[Int](2 * ARRAY_SAMPLE_SIZE)
